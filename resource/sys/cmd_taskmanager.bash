cmd_taskmanager() {
    ...
    while [ $stop -eq 0 ]; do
        ...
        # ---- 计算任务管理器已运行时长 ----
        local current_time_stamp=$($_DATE +%s)
        local elapsed=$((current_time_stamp - start_time))
        ...
        if [ $elapsed -lt 60 ]; then
            ...
            elapsed_formatted="${mins}m ${secs}s"
        else
            local hours=$((elapsed / 3600))
            local mins=$(((elapsed % 3600) / 60))
            local secs=$((elapsed % 60))
            elapsed_formatted="${hours}h ${mins}m ${secs}s"
        fi
        ...
        cecho "时间: $current_time  监测时长: $elapsed_formatted  系统运行时间: $uptime_raw"
        cecho "$CMD_delimiter"
        cecho "$tasks_display"
