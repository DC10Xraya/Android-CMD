...
_display_month_calendar() {
    ...
    # 检查是否有可用的 `cal` 或 `ncal` 命令
    if command -v cal >/dev/null 2>&1; then
        # 优先使用系统的 cal 命令,输出逐行通过 cecho
        cal "$month" "$year" 2>/dev/null | while IFS= read -r line; do cecho "$line"; done
        return
    fi
    if command -v ncal >/dev/null 2>&1; then
        ncal -b "$month" "$year" 2>/dev/null | while IFS= read -r line; do cecho "$line"; done
        return
    fi
    
    # 如果没有 cal/ncal 命令,则用 shell 实现一个简单的日历
    _get_month_calendar_text "$year" "$month" | while IFS= read -r line; do
        cecho "$line"
    done
}

# ---------- 获取单月日历文本(统一格式,支持高亮今日) ----------
_get_month_calendar_text() {
    ...
    local month_names=("一月" "二月" "三月" "四月" "五月" "六月" 
                       "七月" "八月" "九月" "十月" "十一月" "十二月")
    local month_index=$((10#$month - 1))
    local month_name="${month_names[$month_index]}"
    
    # 计算当月天数
    local last_day
    # 先尝试使用 date 命令
    last_day=$(date -d "${year}-${month}-01 +1 month -1 day" +%d 2>/dev/null 2>/dev/null)
    ...
    local first_weekday
    first_weekday=$(date -d "${year}-${month}-01" +%w 2>/dev/null 2>/dev/null)
    if [ -z "$first_weekday" ] || [ $? -ne 0 ]; then
        # 如果 date 命令失败,使用蔡勒公式计算
        local y=$((year % 100)) m=$((10#$month)) c=$((year / 100))
        if [ $m -le 2 ]; then
            m=$((m + 12))
            y=$((y - 1))
        fi
