...
cmd_portscan() {
    if [ "$1" = "-h" ] || [ "$1" = "--help" ] || [ $# -eq 0 ]; then
        cecho -b "用法: PORTSCAN [-t 线程数] [-W 超时(s)] [-i 间隔(s)] <域名/IP> <开始端口> <结束端口>"
        cecho "示例: PORTSCAN 110.42.107.47 25500 25600"
        return 0
    fi

    local target start_port end_port timeout_sec=1 interval=0.2 max_workers=8
    ...
    if ! echo "$target" | grep -qE "$ip_regex"; then
        ...
        if command -v getent >/dev/null 2>&1; then
            resolved_ip=$(getent ahosts "$target" 2>/dev/null | head -1 | awk '{print $1}')
        fi

        if [ -z "$resolved_ip" ] && command -v nslookup >/dev/null 2>&1; then
            resolved_ip=$(nslookup "$target" 2>/dev/null | grep -E "^Address:" | tail -1 | awk '{print $2}')
        fi

        if [ -z "$resolved_ip" ] && command -v busybox >/dev/null 2>&1; then
            resolved_ip=$(busybox nslookup "$target" 2>/dev/null | grep -E "^Address:" | tail -1 | awk '{print $2}')
        fi

        if [ -z "$resolved_ip" ] && command -v ping >/dev/null 2>&1; then
            resolved_ip=$(ping -c1 "$target" 2>/dev/null | head -1 | sed -n 's/.*(\([0-9.]\{7,15\}\)).*/\1/p')
        fi
        ...
        if [ -z "$resolved_ip" ]; then
            if command -v curl >/dev/null 2>&1 || command -v wget >/dev/null 2>&1; then
                ...
                resolved_ip=$(echo "$json" | grep -o '"data":"[^"]*"' | head -1 | sed 's/"data":"//; s/"//')
            else
                err "未找到 curl 或 wget, 且无本地解析工具, 无法解析域名"
            fi
        fi

        if [ -n "$resolved_ip" ] && [[ "$resolved_ip" =~ ^[0-9.]+$ ]]; then
            ip="$resolved_ip"
        else
