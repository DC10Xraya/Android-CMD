...
<details>
<summary><h2>命令 (109) 点击以查看</h2></summary>
...
| UNZIP | 解压 ZIP 压缩包 |
| DUMP | 导出文件夹结构到文本文件 |
| FORMAT | 格式化存储设备（需 root） |
| MOUNT | 挂载文件系统（需 root） |
| UMOUNT | 卸载文件系统（需 root） |
| NOW | 显示当前时间 |
| CAL | 显示日历 |
| CLOCK | 实时时间（0.1s 刷新） |
| FREE | 内存使用情况 |
...
| TEMP | 温度传感器与温控状态 |
| MONITOR | 实时显示时间、内存、温度 |
| CPUMONITOR | 各 CPU 核心实时频率 |
...
| DOWNLOAD | 下载网络文件到本地 |
| ST/SPEEDTEST | 网络测速 |
| MCMODDOWNLOAD | 从 Modrinth 下载 MC 模组 |
...
| REPEAT | 重复执行命令 |
| CMDTIME | 测量命令耗时 |
| MCSERVER | 模拟 MC 服务器运行 |
| FUN/FUNCTION | 临时定义函数（重启失效） |
| CLS/CLEAR | 清屏 |
| CLSD/CLEARD | 设置清屏默认行为 |
| COLOR | 设置控制台颜色 |
| TITLE | 设置终端标题 |
| TMPDIR | 设置/查看临时目录 |
| RESOURCE | 查看/重新加载资源 |
