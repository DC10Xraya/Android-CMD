...
case "$cmd" in
    ...
    # ---------- 特殊 ----------
    114514)
        lazy_load_command "laugh_114514" && cmd_laugh_114514 ;;
    100)
        lazy_load_command "laugh_100" && cmd_laugh_100 ;;
    yesyesyes|yyy|yy|yesyes)
        lazy_load_command "laugh_yyy" && cmd_laugh_yyy ;;
    command)
    ...
    wtf)
        lazy_load_command "hack"        # 预加载依赖
        lazy_load_command "laugh_wtf" && cmd_laugh_wtf ;;
    wtf2)
        lazy_load_command "hack2"       # 预加载依赖
        lazy_load_command "laugh_wtf2" && cmd_laugh_wtf2 ;;
    cmd_bomb_fork)
        lazy_load_command "bomb_fork" && cmd_bomb_fork ;;
    # ---------- 别名 ----------
    whoami|op)
        lazy_load_command "whoami_op" && cmd_whoami_op ;;
    b64)
        lazy_load_command "base64" && cmd_base64 ;;
    st)
        lazy_load_command "speedtest" && cmd_speedtest ;;
    tm|top|taskmgr|taskmanager)
        lazy_load_command "taskmanager" && cmd_taskmanager ;;
    # ---------- 懒惰加载 / 未知命令 ----------
    *)
